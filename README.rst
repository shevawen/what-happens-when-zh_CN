...
浏览器读取这个URL后，浏览器会展示网页中被命名为 print 的内容，默认效果是滚动至该区域。
``#`` 是用来指导浏览器动作的，对服务器端完全无用。所以，HTTP请求中不包括 ``#`` 。请求该地址，浏览器实际发出的请求是这样的::

    GET /index.html HTTP/1.1
    Host: www.example.com

可以看到，只是请求index.html，根本没有 ``print`` 的部分。在单页web应用(Single Page Application)中，``#`` 作为页内导航的手段。
